	...
	<aside class="sidebar">
		...
		<nav class="sidebar-nav">
			<template v-for="section in sections" :key="section.key">
				<a v-if="section.external" :href="section.path" target="_blank" rel="noopener" class="sidebar-item">
					<div class="sidebar-icon" v-html="section.icon"></div>
					<span class="sidebar-tooltip">{{ section.label }}</span>
				</a>
				<router-link v-else :to="section.path" class="sidebar-item" :class="{ active: isActive(section) }">
					<div class="sidebar-icon" v-html="section.icon"></div>
					<span class="sidebar-tooltip">{{ section.label }}</span>
				</router-link>
			</template>
...
const sections = [
	...
	{ key: 'inbox', label: 'Inbox', path: '/roundcube', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/></svg>', external: true },
...
function isActive(section) {
	...
	switch (section.key) {
		...
		case 'campaigns': return p.startsWith('/sequences') || p.startsWith('/market')
